...
recvTMP1200_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_300:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown"));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_BADCONNECTION:
			if (buf[0])
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection"));
			else
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: RTD is likely disconnected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
sendDCC1004(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1004_VOLTAGEINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DCC1004_MOTORVELOCITYCONTROLLER_100:
		switch (bp->vpkt) {
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETDEADBAND, buf, 4));
...
sendDCC1101(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_DCC1101_CURRENTINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DCC1101_VOLTAGEINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DCC1101_TEMPERATURESENSOR_IC_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DCC1101_ENCODER_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DCC1101_DCMOTOR_100:
		switch (bp->vpkt) {
		case BP_SETACCELERATION:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORCONTROLLER_SETACCELERATION, buf, 4));
	...
	case PHIDCHUID_DCC1101_MOTORPOSITIONCONTROLLER_100:
		switch (bp->vpkt) {
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETDEADBAND, buf, 4));
	...
	case PHIDCHUID_DCC1101_MOTORVELOCITYCONTROLLER_100:
		switch (bp->vpkt) {
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_SETDEADBAND, buf, 4));
...
static PhidgetReturnCode
recvDCC1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DCC1101_DCMOTOR_100:
		...
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			...
			case 1:
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
			case 2:
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_ESTOP, "Emergency stop procedure initiated."));
			case 3:
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Verify motor is not moving and check connections. Close the Motor Controller and re-open it to try again."));
			case 4:
...
#endif /* PHIDUID_DCC1101_SUPPORTED */
